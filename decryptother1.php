...
$sql = "SELECT id,n,d,mcipher FROM user_ciphertext where umail='$email'";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) > 0) {
    // output data of each row
	echo "<table background='back.jpg'><tr><td>ID</td><td>N</td><td>SECRET CIPHER</td></tr><tr>";
    while($row = mysqli_fetch_assoc($result)) {
        echo "<td> " . $row["id"]. "</td><td>".$row["n"]."<td><textarea type='text' width='400px'>".$row["mcipher"]."</textarea></td></tr>";
    }
	echo"</table>";
} else {
    echo "0 results";
...
Copy and Paste Secret Cipher Text Here:</td><td><textarea type="text" required="true" name="secrettext" value="" style="width:300px;height:100px;"></textarea></td></tr>
<tr><td>Enter Private Key:</td><td><input type="number" name="k" required="true"></td></tr>
<tr><td>Enter N value:</td><td><input type="number" required="true" name="n"></td></tr>
<tr></tr>
...
$sql = "SELECT * FROM user_ciphertext where umail='$email' and mplain='$auth'";
...
$count = mysqli_num_rows($result);
	...
	if($count > 0)
	{
		...
		echo"<script>if(confirm('You are Genuine,Authentication Successfull.')){ document.location = 'http://127.0.0.1/final/laststep1.php';}</script>";
		}
	else
	{
		echo"<script>alert('Sorry! Authentication is Failed!!!! Please Try Again!')</script>";
	}
	mysqli_close($conn);
}
?>
